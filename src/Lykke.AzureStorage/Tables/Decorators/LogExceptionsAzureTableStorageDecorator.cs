...
namespace AzureStorage.Tables.Decorators
{
        ...
        where TEntity : ITableEntity, new()
    {
        ...
        public Task<bool> InsertOrReplaceAsync(TEntity entity, Func<TEntity, bool> replaceCondition)
            => WrapAsync(() => _impl.InsertOrReplaceAsync(entity, replaceCondition), nameof(InsertOrReplaceAsync), entity);
        ...
        public Task<bool> DeleteIfExistAsync(string partitionKey, string rowKey, Func<TEntity, bool> deleteCondition)
            => WrapAsync(() => _impl.DeleteIfExistAsync(partitionKey, rowKey, deleteCondition), nameof(DeleteIfExistAsync), new { partitionKey, rowKey });
        ...
        public Task GetDataByChunksAsync(string partitionKey, Func<IEnumerable<TEntity>, bool> chunkHandler)
            => WrapAsync(() => _impl.GetDataByChunksAsync(partitionKey, chunkHandler), nameof(GetDataByChunksAsync), new { partitionKey });
        ...
        public Task<(IEnumerable<TEntity> Entities, string ContinuationToken)> GetDataWithContinuationTokenAsync(TableQuery<TEntity> rangeQuery, string continuationToken)
            => WrapAsync(() => _impl.GetDataWithContinuationTokenAsync(rangeQuery, continuationToken), nameof(GetDataWithContinuationTokenAsync));
        ...
        public Task<TEntity> GetTopRecordAsync(TableQuery<TEntity> query)
            => WrapAsync(() => _impl.GetTopRecordAsync(query), nameof(GetTopRecordAsync), query.FilterString);

        public Task<IEnumerable<TEntity>> GetTopRecordsAsync(TableQuery<TEntity> query, int n)
            => WrapAsync(() => _impl.GetTopRecordsAsync(query, n), nameof(GetTopRecordsAsync), new { query.FilterString, n });
